...
class Track:
    ...
    def sphericalToDecart(self, azimuth: float, elevation: float) -> Tuple[float, float]:
        ...
        # cos - y ==> North 
        # sin - x ==> West
        x = -(self.heigth / tan(elevation)) * cos(azimuth)
        y =  (self.heigth / tan(elevation)) * sin(azimuth)
    ...
    def getNearestPass(self, satellite: str) -> Tuple[Tuple[datetime, datetime, datetime], str]:
        return (self.getOrbital(satellite).get_next_passes(datetime.utcnow(), 48, self.lon, 
            self.lat, self.alt, horizon=50)[0],satellite)
    
    def generateTrack(self)-> Tuple[str, datetime, datetime, List[Tuple[float, float, float]]]:
        ...
        for i in range((satPass[1] - satPass[0]).seconds):
            ...
            big_elev = max(big_elev, look[1])
            x, y = self.sphericalToDecart(*look)
            xCoords.append(x)
            yCoords.append(y)
            zCoords.append(self.heigth)
        
        return satellite, satPass[0], satPass[-1], list(zip(xCoords, yCoords, zCoords)), big_elev      
